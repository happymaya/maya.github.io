---
title: Redis崩溃后，如何进行数据恢复的
author:
  name: superhsc
  link: https://github.com/happymaya
date: 2021-07-01 24:33:22 +0800
...
通过 RDB、AOF、混合存储等数据持久化方案解决如何进行数据恢复的问题。

![](https://images.happymaya.cn/assert/db/cache/cache-redis-0901.png)
...
4. 第四种场景是在运维执行 flushall 清空所有数据，或执行 shutdown 关闭服务时，也会触发 Redis 自动构建 RDB 快照。
   ...
   ![](https://images.happymaya.cn/assert/db/cache/cache-redis-0902.png)
...
![](https://images.happymaya.cn/assert/db/cache/cache-redis-0903.png)
...
![](https://images.happymaya.cn/assert/db/cache/cache-redis-0904.png)
...
3. 第三种配置策略是 everysec。即每秒通过 BIO 线程进行一次 fsync。这种策略在安全性、性能，以及磁盘寿命之间做较好的权衡，可以较好的满足线上业务需要。
   ...
   ![](https://images.happymaya.cn/assert/db/cache/cache-redis-0905.png)
...
![](https://images.happymaya.cn/assert/db/cache/cache-redis-0906.png)
...
![](https://images.happymaya.cn/assert/db/cache/cache-redis-0907.png)
