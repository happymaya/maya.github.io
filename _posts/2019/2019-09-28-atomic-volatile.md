...
我们首先看一个案例。如图所示，有两个线程。

![](https://images.happymaya.cn/assert/java/thread/java-thread-atomic-volatile-1.png)
...
![](https://images.happymaya.cn/assert/java/thread/java-thread-atomic-volatile-2.png)
...
![](https://images.happymaya.cn/assert/java/thread/java-thread-atomic-volatile-3.png)
...
![](https://images.happymaya.cn/assert/java/thread/java-thread-atomic-volatile-4.png)
...
![](https://images.happymaya.cn/assert/java/thread/java-thread-atomic-volatile-5.png)

如果它被初始化为每个线程都加 1000 次，最终的结果很可能不是 2000。由于 value++ 不是原子的，所以在多线程的情况下，会出现线程安全问题。但是如果我们在这里使用 volatile 关键字，能不能解决问题呢？

![](https://images.happymaya.cn/assert/java/thread/java-thread-atomic-volatile-6.png)
...
![](https://images.happymaya.cn/assert/java/thread/java-thread-atomic-volatile-7.png)
...
![](https://images.happymaya.cn/assert/java/thread/java-thread-atomic-volatile-8.png)
...
> 可以volatile和AtomicInteger组合使用吗？——  不需要，单独使用AtomicInteger已经可以保证原子性了
...
> 如果只使用了synchronized同步，但变量没有用 volite 修饰，也可以保证可见性。