---
title: 正确关闭线程池？shutdown 和 shutdownNow 的区别 ?
author:
  name: superhsc
  link: https://github.com/happymaya
date: 2019-02-17 23:33:00 +0800
...
> 如果只是想中断线程池某个任务呢？这个要怎么操作 ？ - 可以用Future的cancel方法。



> 在实际项目中,模块全局单例线程池可以不关闭吧?关闭之后再次调用,任务提交后并不执行呀,只执行第一次调用时提交的任务啊! —— 可以不关闭。



> 什么情况下需要关闭,线程池不是一直存在的吗 —— 线程池不关闭的话，里面的线程会一直存在。在使用完线程池后，可以选择关闭。



> 想问下">isShutdownisTerminated 这2个方法只是获取状态 还是由关闭的功能？？ 看源码好像并没有关闭功能 只是做了一些判断 —— 是的，没有关闭，只是判断。